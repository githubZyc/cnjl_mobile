...
var img_translate = "../../img/translate.png";
//var img_translate = "_www/img/translate.png";
 ...
 * 格式如下:
 * <img src='默认图片' data-srcs='网络地址' onload='load(this)'/>
 */
function load(obj) {
	if(obj.getAttribute('data-loaded')) return;
	var image_url = obj.getAttribute('data-srcs');
	if(!image_url) return;
	//本地缓存路径
	var hb_path = '_downloads/image/' + image_url + '.jpg'; //HBuilder平台路径
...
//文件是否存在
function FileExists(src, success, fail) {
	var image_url = src;
	if(!image_url) return;
	//本地缓存路径
...
//下载dome不是img的图片
function DownloadFile(src, success, fail) {
	var image_url = src;
	if(!image_url) return;
	//本地缓存路径
	var hb_path = '_downloads/image/' + md5(image_url) + '.jpg'; //HBuilder平台路径
