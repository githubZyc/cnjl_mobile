    ...
    $.fn.pullRefresh = function(options) {
        if (this.length === 1) {
            ...
            if (options.down && options.down.auto) { //如果设置了auto，则自动下拉一次
            	console.info("code line 4744 console");
            	console.info(options.down.cacheKey);
                console.info("localStorage.getItem(options.down.cacheKey):"+plus.storage.getItem(options.down.cacheKey));
                if(options.down.cacheKey&&plus.storage.getItem(options.down.cacheKey)){
                	console.info("自动下拉时存储的key:"+options.down.cacheKey);
                	console.info("自动下拉时存储的value:"+JSON.stringify(plus.storage.getItem(options.down.cacheKey)));
                }else{
    ...
    $.plusReady(function() {
        ...
        $.fn.pullRefresh_native = function(options) {
            ...
            if (options.down && options.down.auto) { //如果设置了auto，则自动下拉一次
                ...
                console.info("code line 5511 console");
                console.info(options.down.cacheKey);
                console.info("localStorage.getItem(options.down.cacheKey):"+plus.storage.getItem(options.down.cacheKey));
                if(options.down.cacheKey&&plus.storage.getItem(options.down.cacheKey)){
                	console.info("自动下拉时存储的key:"+options.down.cacheKey);
